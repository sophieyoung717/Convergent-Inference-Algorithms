...
/**
 * The <tt>Topological</tt> class represents a data type for determining a
 * topological order of a directed acyclic graph (DAG). Recall, a digraph has a
 * topological order if and only if it is a DAG. The <em>hasOrder</em> operation
 * determines whether the digraph has a topological order, and if so, the
 * <em>order</em> operation returns one.
 * <p>
 ...
 * the <em>hasOrder</em> and <em>rank</em> operations takes constant time; the
 * <em>order</em> operation takes time proportional to <em>V</em>.
 * <p>
 * See {@link DirectedCycle}, {@link DirectedCycleX}, and
 * {@link EdgeWeightedDirectedCycle} to compute a directed cycle if the digraph
 * is not a DAG. See {@link TopologicalX} for a nonrecursive queue-based
 * algorithm to compute a topological order of a DAG.
 * <p>
...
public class Topological {
	 ...
	 *         equivalently, if the digraph is a DAG), and <tt>false</tt> otherwise
	 */
	...
	/**
	 * The the rank of vertex <tt>v</tt> in the topological order; -1 if the digraph
	 * is not a DAG
	 * 
